...
Self-contained JMESPath implementation with 470+ extension functions. Provides the parser, runtime, and function registry used by all other jpx crates. This is a from-scratch implementation with no upstream dependency on the unmaintained `jmespath` crate.

- Uses `serde_json::Value` natively -- no type conversion overhead
- 26 standard JMESPath built-in functions + 470+ extensions across 32 extension categories
