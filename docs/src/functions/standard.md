...
| [`not_null`](#not_null) | `any... -> any` | Returns the first non-null argument |
...
| [`sort_by`](#sort_by) | `array, expression -> array` | Sorts an array by expression result |
| [`starts_with`](#starts_with) | `string, string -> boolean` | Returns true if the subject starts with the prefix |
| [`sum`](#sum) | `array[number] -> number` | Returns the sum of an array of numbers |
| [`to_array`](#to_array) | `any -> array` | Converts a value to an array |
| [`to_number`](#to_number) | `any -> number` | Converts a value to a number |
| [`to_string`](#to_string) | `any -> string` | Converts a value to a string |
